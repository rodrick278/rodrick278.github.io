...
1. 那么会在原型链上找到 `hamster`  ： `this`（`=speedy`）中查找 `stomach`  ，然后执行 `eat` 。
1. 然后它对 `stomach` 调用 `push`，将食物添加到 **`stomach` 的原型** 中



对于 `lazy.stomach.push(...)` 和 `speedy.stomach.push()` 而言，属性 `stomach` 被在原型中找到（不是在对象自身），然后向其中 `push` 了新数据。

在简单的赋值 `this.stomach=` 的情况下不会出现这种情况：
```javascript
let hamster = {
  stomach: [],
...
```
因为 `this.stomach=` 不会执行对 `stomach` 的查找。该值会被直接写入 `this` 对象。**<br />
